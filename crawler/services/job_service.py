...
class JobService(BaseMongodbService):
    ...
    def add_task(self, isbn_list):
        """
        Search book-info by isb13,
        then push new task into queue.

        :type isbn_list: object
        """
        # Arguments
        if None is isbn_list:
            raise ValueError('Invalid `isbn13` value(s).')
        if str is isbn_list:
            isbn_list = [isbn_list]
        ...
        with DoubanBookService() as svs:
            ...
            for cur_isbn in isbn_list:
                # Search book and fetch `book_id` list
                book_info = svs.get_book_id_by_isbn13(isbn13=cur_isbn)

                # Force to wait 0.5~3 seconds
                time.sleep(float(rnd.randint(500, 3000)) / 1000.0)
                ...
                if None is not book_info and book_info.get('book_id_list'):
                    ...
                    for book_id in book_id_list:
                        ...
                        if 0 < JobMongoBiz.count(criteria={'status': S_READY, 'job_info': job_info}):
                            print('Found duplicate task, which book-id is "{}".'.format(
                                book_id
                            ))
                            continue
