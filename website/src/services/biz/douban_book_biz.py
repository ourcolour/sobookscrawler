...
class DoubanBookBiz(object):
	...
	def get_collections_info(cls, driver, wait, ref_book) -> bool:
		...
		try:
			...
			collections = int(driver.find_element_by_xpath('//span[@id="collections_bar"]/span').text.replace('人', '').replace('读过', '').strip())
			doings = int(driver.find_element_by_xpath('//span[@id="doings_bar"]/span').text.replace('人', '').replace('在读', '').strip())
			wishes = int(driver.find_element_by_xpath('//span[@id="wishes_bar"]/span').text.replace('人', '').replace('想读', '').strip())

			# Assign value
			ref_book.rating['collections'] = ref_book.build_collections(wishes=wishes, doings=doings, collections=collections)
			pass
		except Exception as ex:
			result = False
			print(ex)
